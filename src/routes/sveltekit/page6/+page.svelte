...
<button onclick={()=>{count++;}}>増やす</button>`}/>
    ...
    <Highlight style="max-width: 1000px; padding-bottom: 10px;" language={vbscriptHtml} code={`<Profile name="Taro" gender="male" birthYear="1940" party="Jimintou"/>`}/>
...
<p>政党:{p.party}</p>`}/>
    ...
    <Highlight style="max-width: 1000px; padding-bottom: 10px;" language={vbscriptHtml} code={`<script lang="ts">
    let {name, gender, birthYear, party} = $props();
<\/script>
...
<Counter/>
<Profile name="Sanae" gender="female" birthYear="1961" party="Jimintou"/>

...
<div>
    ...
    <Highlight style="max-width: 1000px; padding-bottom: 10px;" language={bash} code={'npm i -D @sveltejs/adapter-cloudflare'}/>
    <p style="padding-bottom: 10px;">できたら、プロジェクトのルートにあるsvelte.config.jsの内容を以下のものに書き換えます。</p>
    <Highlight style="max-width: 1000px; padding-bottom: 10px;" language={javascript} code={`import adapter from '@sveltejs/adapter-cloudflare';
...
export default config;`}/>
    ...
    <Pager prevTitle="前へ" prevURL={resolve("/sveltekit/2-deploy-to-github")} nextTitle="次へ" nextURL={resolve("/sveltekit/4-svelte-start")}/>
</div>
